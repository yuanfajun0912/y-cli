...
function mmaster (message, weeks = 2) {
  ...
  mergeCommits(message, weeks)
  compareDiff(selfBranch, bakBranch)
  exec('git push -f')
  success(`执行 y mmaster "${message}" 成功，去新建merge request~`)
}
...
function compareDiff (selfBranch, bakBranch) {
  ...
  if (diffRes) {
    diffSpinner.succeed(`分支${selfBranch}和分支${bakBranch}的改动差异一致`)
  } else {
    diffSpinner.fail(`分支${selfBranch}和分支${bakBranch}的改动差异不一致`)
    exitProcess();
  }
}
